...
| Pi Coding Agent CLI / Oh My Pi | `~/.pi/agent/sessions/<project>/*.jsonl` + `~/.omp/agent/sessions/<project>/*.jsonl` | JSONL, `message.usage` |
...
**Pi Coding Agent CLI / Oh My Pi** — 字段独立,与 OpenCode 展示口径一致:
- 输入 = `usage.input`
- 输出 = `usage.output`
- 缓存读 = `usage.cacheRead`
- 缓存写 = `usage.cacheWrite`
- 推理 = `usage.reasoning` / `usage.reason` / `usage.reasoningTokens`(如果存在；Oh My Pi 的 `reasoningTokens` 计入 Pi 推理)
...
### Pi Coding Agent CLI / Oh My Pi / OpenCode 成本

Pi / Oh My Pi 优先使用会话 JSONL 中的 `usage.cost.total`；OpenCode 直接使用消息 JSON 中的 `cost` 字段。若 Pi 成本字段缺失，则按统一价格表用 input/output/cache_read/cache_write 回退估算。
